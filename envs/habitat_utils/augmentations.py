...
class PhotometricDistort(object):
    ...
    def __call__(self, images):
        imgs = images.copy()
        imgs = self.rand_brightness(imgs)
        if random.randint(2):
            distort = Compose(self.pd[:-1])
        else:
            distort = Compose(self.pd[1:])
        imgs = distort(imgs)
        imgs = self.rand_contrast(imgs)
        imgs = self.rand_light_noise(imgs)
        return imgs
        '''
        for image in images:
            im = image.copy()
...
class RPFAugmentation(object):
    def __init__(self, size=64, mean=(104, 117, 123)):
        self.mean = mean
        self.size = size
