...
## ---------------------------------
from multi_env import DeepmindLabEnv, DeepLabPreprocessVectorEnv, _make_env_fn
from deeplab_env import DeepLabMultiWrapper
...
from Policy import PolicyWithBase
...
def main(cfg):
    ...
    actor_critic = PolicyWithBase(perception_model, action_space=envs.action_space, num_stack=cfg.network.num_stack)
    ...
    for epoch in range(start_epoch, num_updates, 1):
        ...
        for step in range(cfg.RL.NUM_STEPS):
            ...
            with torch.no_grad():
                value, action, action_log_prob, pre_embedding = actor_critic.act(obs_unpacked,mask_done.cuda(), mode=training_mode)
