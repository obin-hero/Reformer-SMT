...
from evkit.models.actor_critic_module import NaivelyRecurrentACModule, RoomObjRecurrentModule
from evkit.preprocess import TransformFactory
import evkit.rl.algo
from evkit.rl.policy import Policy, PolicyWithBase, BackoutPolicy, customPolicyWithBase
from evkit.rl.storage import RolloutSensorDictStorage, RolloutSensorDictReplayBuffer, StackedSensorDictStorage
from evkit.saving.checkpoints import save_checkpoint, last_archived_run
from evkit.saving.observers import FileStorageObserverWithExUuid
from evkit.utils.misc import Bunch, cfg_to_md, compute_weight_norm, is_interactive, update_dict_deepcopy
...
def run_training(cfg, uuid):
    try:
        ...
        if agent == None:
            room_perception_model = eval(cfg['learner']['room_perception_network'])(
                              cfg['learner']['num_stack'],
                              **cfg['learner']['room_perception_network_kwargs'])
            obj_perception_model = eval(cfg['learner']['obj_perception_network'])(
                              cfg['learner']['num_stack'],
                              **cfg['learner']['obj_perception_network_kwargs'])
            ...
            base = RoomObjRecurrentModule(
                              room_nav_perception=room_perception_model,
                              obj_nav_perception=obj_perception_model,
                              use_gru=cfg['learner']['recurrent_policy'],
                              internal_state_size=cfg['learner']['internal_state_size'],
                              room_action_space=action_space,
                              obj_action_space=action_space)
            actor_critic = customPolicyWithBase(
                              base, action_space,
        ...
        if 'habitat' in cfg['env']['env_name'].lower() or True:
            for metric in ['metrics/spl', 'metrics/success', 'metrics/visited_cells']:
        ...
        mlog.add_meter('config', tnt.torchnet.meter.SingletonMeter(), ptype='text')
        ...
        for j in range(start_epoch, num_updates, 1):
            for step in range(cfg['learner']['num_steps']):
                ...
                #print('actions {}, done {}'.format(cpu_actions,done))
                #envs.render('human')
                #print(cpu_actions)
                #if info[0]['room_success'] : actor_critic.base.nav_mode = 'obj'
                ...
                #if mem - last_mem > 1 : print('[MEMORY DEBUG] after loggin loop : ',mem - last_mem) 
                #last_mem = mem
                ...
                #mem = process.memory_info().rss
                #if mem - last_mem > 1 : print('[MEMORY DEBUG] current train obs: ',mem - last_mem) 
                #last_mem = mem
                current_obs.insert(obs, mask_done)
                #mem = process.memory_info().rss
                #if mem - last_mem > 1 : print('[MEMORY DEBUG] current train obs insert: ',mem - last_mem) 
                #last_mem = mem
                ...
                #mem = process.memory_info().rss
                #if mem - last_mem > 1 : print('[MEMORY DEBUG] rollout insert: ',mem - last_mem) 
                #last_mem = mem
                mlog.update_meter(value[:num_train_processes].mean().item(), meters={'diagnostics/value'}, phase='train')

                #mem = process.memory_info().rss
                #if mem - last_mem > 1 : print('[MEMORY DEBUG] after insert rollout : ',mem - last_mem) 
                #last_mem = mem
            # Training update
